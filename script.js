...
// Scoring rules
const WRONG_PENALTY_PER_ANSWER = 1;      // –1 per wrong
const UNANSWERED_PENALTY_PER_3 = 1;      // –1 per 3 unanswered

// ================== QUESTION BANK ==================
// Example structure (add your full bank in the same format)
const questionBank = [
  // ===== 3 MARK QUESTIONS =====
   {
      "id": "Q1",
  ...
  // ➜ Add the rest of your questions here in the same structure.
];
...
function pickQuestionsForExam() {
  ...
  // You can customize this later (e.g. 20×3M + 10×4M).
  chosenQuestions = [...questionBank];
  shuffleArray(chosenQuestions);

  const totalQ = chosenQuestions.length;
  setsCount = Math.min(3, Math.ceil(totalQ / 10)); // 3 sets → 30 questions max
}
...
function computeResult() {
  ...
  const wrongPenalty = wrongCount * WRONG_PENALTY_PER_ANSWER; // –1 each wrong
  const unansweredPenalty =
    Math.floor(unansweredCount / 3) * UNANSWERED_PENALTY_PER_3; // –1 per 3 unanswered
...
function showResult(score, autoSubmitted, reasonCode) {
  ...
  if (autoSubmitted && reasonCode === "violation") {
    ...
    if (resultBadgeBox) {
      let html =
        `<p class="violationTitle">Your session was auto-submitted due to security violations (camera/mic/noise/tab switching or restricted actions).</p>`;
...
function startExam() {
  ...
  // Fill student info in exam header
  if (currentUser) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Admin Reset (global 20-day lock reset using code only)
  if (adminResetBtn) {
    adminResetBtn.addEventListener("click", () => {
      ...
      // Clear all 20-day locks and last stored result
      const keysToRemove = [];
