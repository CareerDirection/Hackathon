...
// ================== QUESTION BANK ==================
//
// You can extend this array with all your questions.
...
// }
//
// Currently includes a sample set. Add more following this pattern.
//

const questionBank = [
  // ------- 3 MARK QUESTIONS (at least 20 needed) -------
   {
      "id": "Q1",
      "marks": 3,
  ...
  // ------- 4 MARK QUESTIONS (at least 10 needed) -------
  ...
  // ➜ You can paste the rest of your 160+ questions here
];
...
function updateTimerDisplay() {
  ...
  if (timerLabel) timerLabel.textContent = "Time Left: " + t;
  if (overlayTime) overlayTime.textContent = "Time Left: " + t;
}
...
async function startCameraAndMic() {
  try {
    ...
    const proctorPanel = $("proctorPanel");
    if (proctorPanel) proctorPanel.classList.remove("hidden");
...
// Audio monitor: loud sound for >3 sec → warning
...
// Brightness monitor: camera covered / too dark
    ...
    brightnessCheckId = setInterval(() => {
      ...
      if (avgBrightness < BRIGHTNESS_THRESHOLD) {
        ...
        if (lowBrightnessCount >= REQUIRED_CHECKS) {
          issueWarning("Camera image too dark/covered. Ensure your face is visible.");
...
function issueWarning(message) {
  ...
  const overlayWarn = $("overlayWarnings");
  if (overlayWarn) overlayWarn.textContent = `Warnings: ${warnings} / ${MAX_WARNINGS}`;
...
// Visibility / blur = tab change / alt+tab
...
// Block copy/paste/right-click/select/shortcuts
...
document.addEventListener("copy", (e) =>
  blockAndWarn(e, "Copy is not allowed during the test.")
);
document.addEventListener("cut", (e) =>
  blockAndWarn(e, "Cut is not allowed during the test.")
);
document.addEventListener("paste", (e) =>
  blockAndWarn(e, "Paste is not allowed during the test.")
);
...
document.addEventListener("keydown", (e) => {
  ...
  if (e.ctrlKey && !e.shiftKey && ["c", "x", "v", "s"].includes(key)) {
    blockAndWarn(e, "Keyboard shortcuts like Ctrl+C/X/V/S are not allowed.");
    return;
  }
  ...
  if (
    ...
    blockAndWarn(e, "Developer tools / Inspect is not allowed.");
    return;
  }
...
function lockKeyForEmail(email) {
  return "hackathon_lock_" + email.toLowerCase();
}
...
function persistResult(score, autoSubmitted, reasonCode) {
  ...
  localStorage.setItem("hackathon_last_email", emailLower);
  localStorage.setItem("hackathon_last_result", JSON.stringify(payload));
  setLockForEmail(emailLower);
}

function tryResumeResultFromStorage() {
  const lastEmail = localStorage.getItem("hackathon_last_email");
  const lastResultStr = localStorage.getItem("hackathon_last_result");
  if (!lastEmail || !lastResultStr) return false;

  if (!isLocked(lastEmail)) {
    // Lock expired → allow new attempt, do not auto-show result
    return false;
  }
  ...
  try {
    ...
    showResult(data.score, data.autoSubmitted, data.reasonCode || "normal", true);
    return true;
  } catch (e) {
    return false;
  }
...
function pickQuestionsForExam() {
  ...
  if (threeMark.length < 20 || fourMark.length < 10) {
    alert(
      "Not enough questions in questionBank. Need at least 20 (3-mark) and 10 (4-mark) questions."
    );
    chosenQuestions = [];
    return;
  }
...
function renderCurrentSet() {
  ...
  if (setInfo) setInfo.textContent = `Set ${currentSetIndex + 1} of 3`;
  ...
  questions.forEach((q, idx) => {
    ...
    q.options.forEach((opt) => {
      const val = opt.trim().charAt(0); // 'A','B',...
  ...
  if (currentSetIndex < 2) {
    if (nextSetBtn) nextSetBtn.classList.remove("hidden");
...
function computeResult() {
  ...
  // no negative for unanswered
  let finalScore = correctMarks - penaltyWrong;
...
// ================== RESULT RENDER ==================
function showResult(finalScore, autoSubmitted, reasonCode, fromStorage = false) {
  quizFinished = true;
  inExam = false;
  ...
  const resultCandidate = $("resultCandidate");
  const resultCandidateExtra = $("resultCandidateExtra");
  if (resultCandidate && currentUser) {
    resultCandidate.textContent = `Name: ${currentUser.name}, Email-ID: ${currentUser.email}`;
  }
  if (resultCandidateExtra && currentUser) {
    resultCandidateExtra.textContent = `College: ${currentUser.college} | Stream: ${currentUser.department}`;
  }
  ...
  if (autoSubmitted && reasonCode === "violation") {
    // Violation → no badge, show reasons
    if (vBox) {
    ...
    // Normal completion → badge visible
    if (badgeArea && badgeLabel) {
      if (finalScore >= 90) {
  ...
  if (scoreText) scoreText.textContent = `Your Score: ${finalScore} / 100`;

  let remark = "";
  if (autoSubmitted && reasonCode === "violation") {
    remark =
      "Your session was auto-submitted due to security violations. Please contact support for one more chance if justified.";
  } else if (finalScore >= 90) {
    remark =
      "Congratulations! You are now Eligible for 2nd round interview for Paid Internship.";
  } else if (finalScore >= 70) {
    remark =
...
function finishQuiz(reasonCode) {
  ...
  // save in storage + lock
  persistResult(finalScore, autoSubmitted, reasonCode);
  // show UI
  showResult(finalScore, autoSubmitted, reasonCode, false);
}
...
function startExam() {
  ...
  const overlayWarn = $("overlayWarnings");
  if (overlayWarn) overlayWarn.textContent = `Warnings: 0 / ${MAX_WARNINGS}`;
  const banner = $("warningBanner");
  if (banner) banner.classList.add("hidden");

  const examNameLine = $("examNameLine");
  const examStatusLine = $("examStatusLine");
  const userInfoDisplay = $("userInfoDisplay");
  const overlayCandidate = $("overlayCandidate");

  if (examNameLine && currentUser) {
    examNameLine.textContent = `Name: ${currentUser.name}, Email-ID: ${currentUser.email}`;
  }
  if (examStatusLine) {
  ...
  if (userInfoDisplay && currentUser) {
    userInfoDisplay.textContent = `${currentUser.name} | ${currentUser.email}`;
  }
  if (overlayCandidate && currentUser) {
  ...
  if (chosenQuestions.length !== 30) {
    alert(
      "Error: Could not pick 30 questions. Check questionBank marks distribution (20×3M + 10×4M)."
    );
    return;
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // If previous result exists & lock active → show result page directly
  if (tryResumeResultFromStorage()) {
    return; // already switched to resultSection
  }
  ...
  if (userForm) {
    userForm.addEventListener("submit", (e) => {
      ...
      if (isLocked(email)) {
        ...
        // If we have stored result for this email, we show it:
        const lastEmail = localStorage.getItem("hackathon_last_email");
        const lastResultStr = localStorage.getItem("hackathon_last_result");
  ...
  if (adminResetBtn) {
    adminResetBtn.addEventListener("click", () => {
      ...
      if (!studentEmail || !code) {
        alert("Enter student email and admin reset code.");
        return;
      }
      ...
      const key = lockKeyForEmail(studentEmail);
      localStorage.removeItem(key);

      const lastEmail = localStorage.getItem("hackathon_last_email");
      if (lastEmail === studentEmail) {
        localStorage.removeItem("hackathon_last_email");
        localStorage.removeItem("hackathon_last_result");
      }

      alert(
        `Reset successful for ${studentEmail}. They can attempt the Hackathon again.`
      );
    });
  }
});
